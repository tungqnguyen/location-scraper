...
const googleDirectionLinks = {
    ...
    extractFromGoogleDirectionsLink(url) {
        ...
        params.forEach((value, key) => {
            found = key.match(/query|query_place_id|origin|origin_place_id|destination|destination_place_id|center|viewpoint|waypoints|waypoint_place_id/);
            if(found) {
                //if value is a lat long pair
                let latLon = this.extractLatLon(value);
                if(latLon) {
    ...
    extractFromGoogleMapLinks(url){
        ...
        let found = url.match(/(place|dir)\/+(.+)\/\@(-?\d+.?\d+),(-?\d+.?\d+)/);
        if (found) { 
            locationFound = {};       
    ...
    scrapeLocation({url}) {
        ...
        let result = Util.formatData(locationData, 'ggMap');
        if (result.length) {
            return result;
        }        
        return [];
