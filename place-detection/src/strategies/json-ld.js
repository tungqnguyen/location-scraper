...
const jsonLd = {
    async scrapeLocation({url}) {
        try {
            ...
            if(object.hasOwnProperty('json-ld')) {
                let jsonLdObject = object['json-ld'];
                ...
                collection.map((el) => {
                    locationObjects = el.filter((object, i) => {
                    ...
                    placeName.map(placeObject => {
                            locationInfo.map(v => {
                                ...
                                let parentPath = v.path.match(/.*\./i)[0].slice(0, -1);
            ...
            let result = Util.formatData(locationCollection, 'jsonLd')
            if (result.length) {
                return result;
            } 
            return [];
    ...
    concatAddress(addressObject) {
        let address = ''
        let addressProps = ['addressCountry','addressLocality','addressRegion','postOfficeBoxNumber','postalCode','streetAddress']
