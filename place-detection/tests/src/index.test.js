...
describe('Extract from Web resource/jsonLd strategy', () => {
  test('Will parse in a web url', async () => {
    ...
    expect(data).toEqual([{
      lat: -31.956376975, lon: 141.468313336, rank: 8, foundFrom: [{ source: 'ggMap', count: 4 }],
    }, {
      lat: -31.9574, lon: 141.4651, rank: 2, foundFrom: [{ source: 'generalRegEx', count: 2 }],
    }, {
      lat: -31.9594976, lon: 141.4634907, rank: 2, foundFrom: [{ source: 'generalRegEx', count: 2 }],
    }, {
      lat: -31.9567631, lon: 141.4629789, rank: 2, foundFrom: [{ source: 'generalRegEx', count: 2 }],
    }]);
  });
  ...
  test('extract lat/lon from another web url', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      lat: -36.7656786, lon: 144.2822309, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }]);
  });
  ...
  test('extract lat/lon from another web url', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      lat: -33.895161, lon: 151.27436, rank: 2, foundFrom: [{ source: 'generalRegEx', count: 2 }],
    }]);
  });
  ...
  test('extract lat/lon from another web url', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      lat: -33.866202, lon: 151.210188, rank: 6, foundFrom: [{ source: 'generalRegEx', count: 3 }, { source: 'jsonLd', count: 1 }], name: 'Rockpool Bar & Grill', path: 'jsonLd[0]', location: '66 Hunter St Sydney 2000 AU ',
    }]);
  });
  ...
  test('extract lat/lon inside jquery functions from web url ', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      lat: -38.48987570840978, lon: 145.20252227783203, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }]);
  });
  ...
  test('extract lat/lon from uberEats', async () => {
    ...
    expect(data[0]).toEqual({
      lat: -37.876475, lon: 145.041992, rank: 5, foundFrom: [{ source: 'generalRegEx', count: 2 }, { source: 'jsonLd', count: 1 }], name: 'Derby Thai Caulfield', path: 'jsonLd[0]', location: '4 Derby Rd Caulfield East VIC 3145 AU ',
    });
  }, 10000);
  ...
  test('extract from hotels.com', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      lat: -37.80929, lon: 144.962711, rank: 4, foundFrom: [{ source: 'generalRegEx', count: 1 }, { source: 'jsonLd', count: 1 }], name: 'Brady Hotel Central Melbourne', path: 'jsonLd[0]', location: 'Australia Melbourne VIC 3000 30 Little La Trobe Street ',
    }]);
  }, 10000);
  ...
  test('extract from The Royal Melbourne Hospital ', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      lat: -37.799615, lon: 144.956562, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }, {
      lat: -37.7988963, lon: 144.9550146, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }, {
      lat: -37.798052, lon: 144.956896, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }, {
      lat: -37.7788135, lon: 144.9467542, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }, {
      lat: -37.778827, lon: 144.948609, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }, {
      lat: -37.778646, lon: 144.947702, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }, {
      lat: -37.777899, lon: 144.947358, rank: 1, foundFrom: [{ source: 'generalRegEx', count: 1 }],
    }]);
  });
});
...
describe('Google Directions Strategy', () => {
  test('Will parse a variation of get directions by URL', async () => {
    ...
    expect(data)
      .toEqual([{
        lat: -33.4545414, lon: 151.28826979999997, rank: 2, foundFrom: [{ source: 'ggMap', count: 1 }],
      }]);
  });
  ...
  test('Will parse a google directions URL directly from google', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      name: 'Alpaca Travel, 17 Marine Parade, Abbotsford VIC 3067', lat: -37.8034086, lon: 144.9986108, rank: 2, foundFrom: [{ source: 'ggMap', count: 1 }],
    }]);
  });
  ...
  test('Will parse a variation of get directions by URL with transportation mode', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      lat: -41.4340812, lon: 147.1373496, rank: 2, foundFrom: [{ source: 'ggMap', count: 1 }],
    }]);
  });
  ...
  test('Will parse a google directions with multiple waypoints URL directly from google', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      name: 'Alpaca Travel, 17 Marine Parade, Abbotsford VIC 3067/Springvale Shopping Centre, Buckingham Avenue, Springvale VIC/North Richmond Railway Station/Victoria St, Abbotsford VIC', lat: -37.8768995, lon: 145.002363, rank: 2, foundFrom: [{ source: 'ggMap', count: 1 }],
    }]);
  });
  ...
  test('Will parse a variation of get directions by URL with transportation mode', async () => {
    ...
    const data = await placeDetection.scrape({ url });
    expect(data).toEqual([{
      name: 'Back Bar', lat: -37.8526825, lon: 144.9907765, rank: 2, foundFrom: [{ source: 'ggMap', count: 1 }],
    }]);
  });
});
